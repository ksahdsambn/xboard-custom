...
Android 1.0 采用两级资产门禁，开发放行与正式验收必须使用不同结论，禁止相互冒充：

- TASK-004 至 TASK-054 属于开发阶段。缺少真机、真实移动网络、真实 VLESS + REALITY 节点、Google Play/RTDN 账号与通道、历史正式 Android 包、生产后端身份或人工合规批准时，允许建立完全隔离、可重复、可审计且明确标记为非生产的开发模拟环境、仿真器、契约服务和测试夹具，验证实现逻辑、错误分支、隔离边界与证据链后继续串行开发。
- 开发模拟资产只能证明“开发测试通过”，不得命名或记录为真机、真实节点、真实 Play 测试轨道、正式 Android 版本、当前生产后端或人工批准，不得用于 Google Play 提审、生产发布或 Android 1.0 完成判定。
- TASK-004 必须建立正式资产延后验收清单，逐项记录真实设备与网络、真实节点、Play/RTDN、当前及上一主要正式版、当前生产后端、发布签名和合规责任。开发期间允许状态为“延后正式验收”，但不得删除、伪造或静默豁免。
- TASK-055 开始正式验收前，必须准备真实服务端、真机、真实移动网络和真实测试节点；TASK-058 必须使用需求 9.3 的正式版本组合；TASK-060 必须取得人工合规与地区批准；TASK-061 必须使用真实 Google Play 内部测试轨道、Developer API 和 RTDN。对应资产缺失时，在首次需要它的正式验收任务阻塞。
- TASK-064 最终完成判定必须确认正式资产延后验收清单为空，并满足本需求第 13 节全部条件；开发模拟结果不能降低任何正式发布门槛。
...
- 在开发阶段先用受控仿真器、开发测试节点和网络故障注入跑通 VpnService、TUN 文件描述符、套接字保护、DNS 控制和 VLESS + REALITY；在阶段 E 用真机和真实测试节点完成正式复验。
- 在开发阶段验证进程重建、Doze、网络切换和资源释放并执行可缩短的长稳测试；阶段 E 仍必须完成真机二十四小时连接及完整门槛。
...
- 使用隔离的开发契约服务完成 Play Billing 全生命周期、Developer API、RTDN 和权益幂等开发验证，并在阶段 E 使用真实 Play 资产复验。
- 完成隐私披露、数据安全表单、账号删除和 VpnService 申报材料草案；人工批准与提审时点复核在阶段 E 完成。

### 阶段 E：验收与发布

- 清空正式资产延后验收清单，使用真机、真实移动网络、真实测试节点、真实 Play/RTDN、正式兼容产物、生产后端身份和人工合规批准完成功能、内核、稳定性、安全、许可证、兼容和回滚演练。
...
- TASK-004 建立的正式资产延后验收清单已经清空，且所有正式证据均来自对应真实资产而非开发模拟环境。
