...
## 2026-09-04 再审查加固（GPT-5 / Codex）

运行器在初始化前废止旧成功结果，输出不得写入官方或定制源码树、非JSON或重解析路径；部分容器创建失败也检查并清理自身标签资源。每轮记录独立runId。Composer逐包比较锁定版本和源码引用，而非仅依赖版本标签。

移动仓验证器直接读取冻结提交的32个Git blob字节计算SHA-256，与两轮证据逐一比较，并完整校验9个当前探针文件的路径集合/哈希、HTTP外层字段、路由与节点摘要白名单。重复文件、错哈希、缺失外层字段、错误路由/镜像与泄漏凭据字段均须被拒绝。

复核实际发现 Windows `core.autocrlf=true` 会改变 `git archive` 的文本字节；运行器已用单次命令配置固定 LF 导出，再重跑两轮，避免同一提交因宿主配置而产生不同源码哈希。没有修改官方或全局 Git 配置。

新增运行器失败分支测试只在临时副本执行，确认缺失上游仓库会使旧成功报告失效、官方/定制源码和非JSON输出不会被覆盖；不修改真实官方源码。原报告的上游行为风险仍由后续对应插件任务规避，本次没有越序实现业务。
