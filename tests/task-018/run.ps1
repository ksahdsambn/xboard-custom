...
[IO.File]::WriteAllText($OutputPath, "{`"taskId`":`"TASK-018`",`"status`":`"失败`",`"tests`":[]}`n", [Text.UTF8Encoding]::new($false))
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
$temporary = Join-Path $temporaryRoot ('xboard-task018-' + [guid]::NewGuid().ToString('N'))
$container = 'xboard-task018-' + [guid]::NewGuid().ToString('N').Substring(0, 12)
$created = $false
$createAttempted = $false
$result = [pscustomobject]@{ schemaVersion = 1; taskId = 'TASK-018'; evidenceClass = 'non-production-simulation'; formalAcceptanceClaimed = $false; sourceCommit = $commit; generatedAt = (Get-Date).ToString('o'); status = '失败'; tests = @() }
...
try {
    ...
    Invoke-Native docker @('run', '-d', '--name', $container, '--label', 'xboard.task=TASK-018', $image, 'sleep', 'infinity') | Out-Null
    $created = $true
    ...
    Invoke-Native docker @('cp', (Join-Path $PSScriptRoot 'audit.php'), "${container}:/audit/task-018-audit.php") | Out-Null
    $phpLint = Invoke-Native docker @('exec', $container, 'sh', '-lc', 'find /audit/plugins/MobileApp/database /audit/plugins/MobileApp/Models -name "*.php" -exec php -l {} \; && php -l /audit/task-018-audit.php')
    Add-Check 'all_migration_php_syntax_valid' ($phpLint -notmatch 'Errors parsing|Parse error' -and $phpLint -match 'No syntax errors detected')
    Invoke-Native docker @('exec', $container, 'sh', '-lc', 'mkdir -p /audit/database && touch /audit/database/task018-prod.sqlite /audit/database/task018-empty.sqlite') | Out-Null
    $raw = & docker exec -w /audit $container sh -lc 'php task-018-audit.php'
    ...
    Add-Check 'runtime_json_status_passed' ($null -ne $audit -and $audit.taskId -eq 'TASK-018' -and $audit.status -eq 'passed') "status=$($audit.status)"
    if ($null -ne $audit) {
...
if ($result.status -ne '通过') { throw 'TASK-018 runtime audit failed; do not advance.' }
Write-Output "TASK-018: $(@($result.tests).Count) checks passed; isolated runtime cleaned."
exit 0
