...
[IO.File]::WriteAllText($OutputPath, "{`"taskId`":`"TASK-017`",`"status`":`"失败`",`"tests`":[]}`n", [Text.UTF8Encoding]::new($false))
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
$temporary = Join-Path $temporaryRoot ('xboard-task017-' + [guid]::NewGuid().ToString('N'))
$container = 'xboard-task017-' + [guid]::NewGuid().ToString('N').Substring(0, 12)
$created = $false
$createAttempted = $false
$result = [pscustomobject]@{ schemaVersion = 1; taskId = 'TASK-017'; evidenceClass = 'non-production-simulation'; formalAcceptanceClaimed = $false; sourceCommit = $commit; generatedAt = (Get-Date).ToString('o'); status = '失败'; tests = @() }
...
try {
    ...
    $fixtureHashes = @(Get-ChildItem -LiteralPath $pluginRoot -Recurse -File | Sort-Object FullName | ForEach-Object {
        [ordered]@{ path = ('plugins/MobileApp/' + [IO.Path]::GetRelativePath($pluginRoot, $_.FullName).Replace('\', '/')); sha256 = (Get-FileHash -LiteralPath $_.FullName -Algorithm SHA256).Hash }
    })
    $createAttempted = $true
    Invoke-Native docker @('run', '-d', '--name', $container, '--label', 'xboard.task=TASK-017', $image, 'sleep', 'infinity') | Out-Null
    $created = $true
    ...
    Invoke-Native docker @('cp', (Join-Path $PSScriptRoot 'audit.php'), "${container}:/audit/task-017-audit.php") | Out-Null
    Invoke-Native docker @('cp', (Join-Path $PSScriptRoot 'task-017-fresh.php'), "${container}:/audit/task-017-fresh.php") | Out-Null
    $phpLint = Invoke-Native docker @('exec', $container, 'sh', '-lc', 'find /audit/plugins/MobileApp -name "*.php" -exec php -l {} \; && php -l /audit/task-017-audit.php && php -l /audit/task-017-fresh.php')
    Add-Check 'all_plugin_php_syntax_valid' ($phpLint -notmatch 'Errors parsing|Parse error' -and $phpLint -match 'No syntax errors detected') $phpLint.Substring(0, [Math]::Min(120, $phpLint.Length))
    Invoke-Native docker @('exec', $container, 'sh', '-lc', 'mkdir -p /audit/database && touch /audit/database/task017.sqlite') | Out-Null
    $raw = & docker exec -w /audit $container sh -lc 'php task-017-audit.php'
    ...
    Add-Check 'runtime_json_status_passed' ($null -ne $audit -and $audit.taskId -eq 'TASK-017' -and $audit.status -eq 'passed') "status=$($audit.status)"
    if ($null -ne $audit) {
        foreach ($item in @($audit.tests)) {
            Add-Check ([string]$item.name) ([bool]$item.passed) (($item.details | ConvertTo-Json -Compress -Depth 4))
        }
    ...
    foreach ($item in $sourceHashes) {
        ...
        if (($runtimeHash -split '\s+')[0].ToUpper() -ne $item.sha256.ToUpper()) { $hashesMatch = $false }
    }
    Add-Check 'official_core_unmodified_in_runtime' $hashesMatch
    ...
    if ($createAttempted -or $created) {
        ...
        $leftoverContainer = docker ps -aq --filter "name=$container"
        $cleaned = [string]::IsNullOrWhiteSpace($leftoverContainer)
    }
    if (Test-Path -LiteralPath $temporary) {
        Get-ChildItem -LiteralPath $temporary -Recurse -Force -ErrorAction SilentlyContinue | ForEach-Object {
...
if ($result.status -ne '通过') { throw 'TASK-017 runtime audit failed; do not advance.' }
Write-Output "TASK-017: $(@($result.tests).Count) checks passed; isolated runtime cleaned."
exit 0
