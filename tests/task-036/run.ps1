...
[IO.File]::WriteAllText($OutputPath, "{`"taskId`":`"TASK-036`",`"status`":`"失败`",`"tests`":[]}`n", [Text.UTF8Encoding]::new($false))
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
$temporary = Join-Path $temporaryRoot ('xboard-task036-' + [guid]::NewGuid().ToString('N'))
$container = 'xboard-task036-' + [guid]::NewGuid().ToString('N').Substring(0, 12)
$created = $false
$createAttempted = $false
$result = [pscustomobject]@{ schemaVersion = 1; taskId = 'TASK-036'; evidenceClass = 'non-production-simulation'; formalAcceptanceClaimed = $false; deviceClaimed = $false; realProductionDeployClaimed = $false; sourceCommit = $commit; generatedAt = (Get-Date).ToString('o'); status = '失败'; tests = @() }
...
try {
    ...
    $scriptsRoot = Join-Path $customRoot 'scripts'
    $createAttempted = $true
    Invoke-Native docker @('run', '-d', '--name', $container, '--label', 'xboard.task=TASK-036', $image, 'sleep', 'infinity') | Out-Null
    $created = $true
    ...
    Invoke-Native docker @('exec', $container, 'sh', '-lc', 'mkdir -p /audit/xboard-custom-scripts') | Out-Null
    Invoke-Native docker @('cp', (Join-Path $scriptsRoot 'deploy-overlay.sh'), "${container}:/audit/xboard-custom-scripts/deploy-overlay.sh") | Out-Null
    Invoke-Native docker @('cp', (Join-Path $scriptsRoot 'update-official-from-git.sh'), "${container}:/audit/xboard-custom-scripts/update-official-from-git.sh") | Out-Null
    Invoke-Native docker @('cp', (Join-Path $scriptsRoot 'update-overlay-from-git.sh'), "${container}:/audit/xboard-custom-scripts/update-overlay-from-git.sh") | Out-Null
    Invoke-Native docker @('cp', (Join-Path $PSScriptRoot 'audit.php'), "${container}:/audit/task-036-audit.php") | Out-Null
    $phpLint = Invoke-Native docker @('exec', $container, 'sh', '-lc', 'php -l /audit/plugins/MobileApp/bin/post-deploy.php && php -l /audit/plugins/MobileApp/Commands/HealthCommand.php && php -l /audit/task-036-audit.php')
    Add-Check 'all_overlay_php_syntax_valid' ($phpLint -notmatch 'Errors parsing|Parse error' -and $phpLint -match 'No syntax errors detected')
    Invoke-Native docker @('exec', $container, 'sh', '-lc', 'mkdir -p /audit/database && touch /audit/database/task036.sqlite') | Out-Null
    $raw = & docker exec -w /audit $container sh -lc 'php task-036-audit.php'
    ...
    Add-Check 'runtime_json_status_passed' ($null -ne $audit -and $audit.taskId -eq 'TASK-036' -and $audit.status -eq 'passed' -and $audit.formalAcceptanceClaimed -eq $false) "status=$($audit.status)"
    if ($null -ne $audit) {
...
if ($result.status -ne '通过') { throw 'TASK-036 runtime audit failed; do not advance.' }
Write-Output "TASK-036: $(@($result.tests).Count) checks passed; isolated runtime cleaned."
exit 0
