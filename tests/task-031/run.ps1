...
[IO.File]::WriteAllText($OutputPath, "{`"taskId`":`"TASK-031`",`"status`":`"失败`",`"tests`":[]}`n", [Text.UTF8Encoding]::new($false))
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
$temporary = Join-Path $temporaryRoot ('xboard-task031-' + [guid]::NewGuid().ToString('N'))
$container = 'xboard-task031-' + [guid]::NewGuid().ToString('N').Substring(0, 12)
$created = $false
$createAttempted = $false
$result = [pscustomobject]@{ schemaVersion = 1; taskId = 'TASK-031'; evidenceClass = 'non-production-simulation'; formalAcceptanceClaimed = $false; deviceClaimed = $false; sourceCommit = $commit; generatedAt = (Get-Date).ToString('o'); status = '失败'; tests = @() }
...
try {
    ...
    Invoke-Native docker @('run', '-d', '--name', $container, '--label', 'xboard.task=TASK-031', $image, 'sleep', 'infinity') | Out-Null
    $created = $true
    ...
    Invoke-Native docker @('cp', (Join-Path $PSScriptRoot 'audit.php'), "${container}:/audit/task-031-audit.php") | Out-Null
    $phpLint = Invoke-Native docker @('exec', $container, 'sh', '-lc', 'find /audit/plugins/MobileApp/Adapters /audit/plugins/MobileApp/Support /audit/plugins/MobileApp/Controllers /audit/plugins/MobileApp/Services /audit/plugins/MobileApp/Models -name "*.php" -exec php -l {} \; && php -l /audit/task-031-audit.php')
    Add-Check 'all_purchase_php_syntax_valid' ($phpLint -notmatch 'Errors parsing|Parse error' -and $phpLint -match 'No syntax errors detected')
    Invoke-Native docker @('exec', $container, 'sh', '-lc', 'mkdir -p /audit/database && touch /audit/database/task031.sqlite') | Out-Null
    $raw = & docker exec -w /audit $container sh -lc 'php task-031-audit.php'
    ...
    Add-Check 'runtime_json_status_passed' ($null -ne $audit -and $audit.taskId -eq 'TASK-031' -and $audit.status -eq 'passed' -and $audit.formalAcceptanceClaimed -eq $false) "status=$($audit.status)"
    if ($null -ne $audit) {
...
if ($result.status -ne '通过') { throw 'TASK-031 runtime audit failed; do not advance.' }
Write-Output "TASK-031: $(@($result.tests).Count) checks passed; isolated runtime cleaned."
exit 0
