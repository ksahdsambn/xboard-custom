...
[IO.File]::WriteAllText($OutputPath, "{`"taskId`":`"TASK-032`",`"status`":`"失败`",`"tests`":[]}`n", [Text.UTF8Encoding]::new($false))
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
$temporary = Join-Path $temporaryRoot ('xboard-task032-' + [guid]::NewGuid().ToString('N'))
$container = 'xboard-task032-' + [guid]::NewGuid().ToString('N').Substring(0, 12)
$created = $false
$createAttempted = $false
$result = [pscustomobject]@{ schemaVersion = 1; taskId = 'TASK-032'; evidenceClass = 'non-production-simulation'; formalAcceptanceClaimed = $false; deviceClaimed = $false; sourceCommit = $commit; generatedAt = (Get-Date).ToString('o'); status = '失败'; tests = @() }
...
try {
    ...
    Invoke-Native docker @('run', '-d', '--name', $container, '--label', 'xboard.task=TASK-032', $image, 'sleep', 'infinity') | Out-Null
    $created = $true
    ...
    Invoke-Native docker @('cp', (Join-Path $PSScriptRoot 'audit.php'), "${container}:/audit/task-032-audit.php") | Out-Null
    $phpLint = Invoke-Native docker @('exec', $container, 'sh', '-lc', 'find /audit/plugins/MobileApp/Adapters /audit/plugins/MobileApp/Support /audit/plugins/MobileApp/Controllers /audit/plugins/MobileApp/Services /audit/plugins/MobileApp/Models /audit/plugins/MobileApp/Http /audit/plugins/MobileApp/Commands -name "*.php" -exec php -l {} \; && php -l /audit/task-032-audit.php')
    Add-Check 'all_rtdn_php_syntax_valid' ($phpLint -notmatch 'Errors parsing|Parse error' -and $phpLint -match 'No syntax errors detected')
    Invoke-Native docker @('exec', $container, 'sh', '-lc', 'mkdir -p /audit/database && touch /audit/database/task032.sqlite') | Out-Null
    $raw = & docker exec -w /audit $container sh -lc 'php task-032-audit.php'
    ...
    Add-Check 'runtime_json_status_passed' ($null -ne $audit -and $audit.taskId -eq 'TASK-032' -and $audit.status -eq 'passed' -and $audit.formalAcceptanceClaimed -eq $false) "status=$($audit.status)"
    if ($null -ne $audit) {
...
if ($result.status -ne '通过') { throw 'TASK-032 runtime audit failed; do not advance.' }
Write-Output "TASK-032: $(@($result.tests).Count) checks passed; isolated runtime cleaned."
exit 0
