...
[IO.File]::WriteAllText($OutputPath, "{`"taskId`":`"TASK-035`",`"status`":`"失败`",`"tests`":[]}`n", [Text.UTF8Encoding]::new($false))
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
$temporary = Join-Path $temporaryRoot ('xboard-task035-' + [guid]::NewGuid().ToString('N'))
$container = 'xboard-task035-' + [guid]::NewGuid().ToString('N').Substring(0, 12)
$created = $false
$createAttempted = $false
$result = [pscustomobject]@{ schemaVersion = 1; taskId = 'TASK-035'; evidenceClass = 'non-production-simulation'; formalAcceptanceClaimed = $false; deviceClaimed = $false; penetrationTestClaimed = $false; sourceCommit = $commit; generatedAt = (Get-Date).ToString('o'); status = '失败'; tests = @() }
...
try {
    ...
    Invoke-Native docker @('run', '-d', '--name', $container, '--label', 'xboard.task=TASK-035', $image, 'sleep', 'infinity') | Out-Null
    $created = $true
    ...
    Invoke-Native docker @('cp', (Join-Path $PSScriptRoot 'audit.php'), "${container}:/audit/task-035-audit.php") | Out-Null
    $phpLint = Invoke-Native docker @('exec', $container, 'sh', '-lc', 'php -l /audit/plugins/MobileApp/Support/MobileSecurityGuard.php && php -l /audit/plugins/MobileApp/Services/SecurityAuditService.php && php -l /audit/task-035-audit.php')
    Add-Check 'all_security_php_syntax_valid' ($phpLint -notmatch 'Errors parsing|Parse error' -and $phpLint -match 'No syntax errors detected')
    Invoke-Native docker @('exec', $container, 'sh', '-lc', 'mkdir -p /audit/database && touch /audit/database/task035.sqlite') | Out-Null
    $raw = & docker exec -w /audit $container sh -lc 'php task-035-audit.php'
    ...
    Add-Check 'runtime_json_status_passed' ($null -ne $audit -and $audit.taskId -eq 'TASK-035' -and $audit.status -eq 'passed' -and $audit.formalAcceptanceClaimed -eq $false -and $audit.penetrationTestClaimed -eq $false) "status=$($audit.status)"
    if ($null -ne $audit) {
...
if ($result.status -ne '通过') { throw 'TASK-035 runtime audit failed; do not advance.' }
Write-Output "TASK-035: $(@($result.tests).Count) checks passed; isolated runtime cleaned."
exit 0
