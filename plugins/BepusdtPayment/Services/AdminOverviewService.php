...
namespace Plugin\BepusdtPayment\Services;

use App\Models\Payment;
use App\Models\Plugin as PluginModel;
use Plugin\BepusdtPayment\Plugin;

class AdminOverviewService
{
    private const PLUGIN_CODE = 'bepusdt_payment';
    private const PAYMENT_METHOD = 'BEpusdt';
    ...
    public function getOverview(): array
    {
        ...
        return [
            ...
            'capabilities' => [
                ...
                'saved_wallet_authorization' => false,
                'automatic_onchain_renewal' => false,
            ],
    ...
    protected function formatInstance(Payment $payment): array
    {
        ...
        $tradeType = trim((string) ($config['trade_type'] ?? ''));
        ...
        return [
            ...
            'status_message' => !$enabled
                ...
                : (empty($missingFields) ? 'BEpusdt instance is ready.' : 'BEpusdt instance is missing required configuration.'),
            'uuid' => $payment->uuid,
            ...
            'runtime_mode' => $tradeType !== '' ? 'create_transaction' : 'create_order',
            'missing_fields' => $missingFields,
            'config_snapshot' => [
                'base_url' => rtrim((string) ($config['base_url'] ?? ''), '/'),
                'fiat' => strtoupper((string) ($config['fiat'] ?? '')),
                'trade_type' => $tradeType !== '' ? $tradeType : null,
                'currencies' => ($tradeType === '' && trim((string) ($config['currencies'] ?? '')) !== '')
                    ? trim((string) $config['currencies'])
                    : null,
                'payment_address' => $tradeType !== '' ? $this->maskSecret((string) ($config['payment_address'] ?? '')) : null,
                'order_name' => $config['order_name'] ?? null,
                'timeout_seconds' => (int) ($config['timeout_seconds'] ?? 0),
                'rate' => trim((string) ($config['rate'] ?? '')) !== '' ? (string) $config['rate'] : null,
                'api_token' => $this->maskSecret((string) ($config['api_token'] ?? '')),
            ],
        ];
    }
